...
class mydataDataset(DatasetTemplate):
    ...
    def get_all_labels(self, num_workers=4, files_list_label=None):
        ...
        def get_single_label_info(single_label_path):
            ...
            print(single_objects_label_info['name'])
            single_objects_label_info['box_size'] = np.array(
                [[label['psr']['scale']['x'], label['psr']['scale']['y'], label['psr']['scale']['z']] for label in
                 labels])  # single_objects_label_info['box_center'] [[4.44983645 1.6537832  1.49163282]]
            ...
            single_objects_label_info['box_center'] = np.array(
                [[label['psr']['position']['x'], label['psr']['position']['y'], label['psr']['position']['z']] for label
                 in labels])
            single_objects_label_info['box_rotation'] = np.array(
                [[label['psr']['rotation']['x'], label['psr']['rotation']['y'], label['psr']['rotation']['z']] for label
                 in labels])
            ...
            single_objects_label_info['gt_boxes'] = np.array([])
            single_objects_label_info['difficulty'] = 0
            if box_center.any():
    ...
    def get_single_pcd_info(self, single_pcd_path):
        ...
        single_pcd_points = pypcd.PointCloud.from_path(
            single_pcd_path)  # /home/algo-4/work/ST3D/data/mydata/lidar/0202.pcd
...
if __name__ == '__main__':
    ...
    if sys.argv.__len__() > 1 and sys.argv[1] == 'create_mydata_infos':
        ...
        # class_names = ['BlueCone', 'YellowCone']
        class_names = ['Cone']
